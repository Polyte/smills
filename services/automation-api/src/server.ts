...
async function main() {
  ...
  app.addHook("preHandler", async (req, reply) => {
    ...
    (req as { automationUserId?: string }).automationUserId = auth.userId;
  });

  app.get("/health", async () => ({ ok: true }));

  app.post<{ Body: { points?: MachineSnapshot[] } }>(
    "/v1/ingest/machine-snapshot",
    async (req, reply) => {
  ...
  }>("/v1/machines/:id/series", async (req, reply) => {
  ...
  app.get<{
    Querystring: { from?: string; to?: string };
  }>("/v1/metrics/oee", async (req, reply) => {
    const to = req.query.to ? new Date(req.query.to) : new Date();
    const from = req.query.from
      ? new Date(req.query.from)
  ...
  app.log.info(
    { port, machines: getSimulatorMachines().map((m) => m.id) },
    "automation-api listening"
  );
}
