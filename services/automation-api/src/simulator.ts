...
import type { MachineSnapshot } from "./db.js";
import { insertSnapshots } from "./db.js";

type MachineSpec = {
  id: string;
  type: "loom" | "knitting" | "dyeing";
  baseRpm: number;
};
...
async function emitSupabaseEvent(
  eventType: string,
  message: string,
...
export async function runSimulatorTick(): Promise<MachineSnapshot[]> {
  const now = new Date().toISOString();
  const batch: MachineSnapshot[] = [];
  ...
  for (const m of MACHINES) {
    ...
    if (rand() < 0.02) st.running = !st.running;
    if (!st.running) {
      st.downtimeAccumulator += 5;
      if (st.downtimeAccumulator >= 900 && rand() < 0.3) {
        st.downtimeAccumulator = 0;
        void emitSupabaseEvent(
          "machine_alarm",
          `${m.id} downtime exceeded 15 minutes — maintenance ticket suggested`,
          { machine_id: m.id, machine_type: m.type }
        );
      }
    } else {
      st.downtimeAccumulator = 0;
  ...
  if (tickCount % 6 === 0) {
    ...
    if (ex) {
      ...
      await emitSupabaseEvent(
        "inventory_automation",
        `Raw cotton received — ${kg} kg (weighbridge @ ${new Date().toISOString().slice(11, 19)})`,
        { source: "simulator", kg }
      );
    }
  }
  if (tickCount % 8 === 0) {
    ...
    await emitSupabaseEvent(
      "inventory_automation",
      `Finished roll #${roll} scanned out for shipping`,
      { source: "simulator", roll }
    );
  }
  if (tickCount % 10 === 0) {
    await emitSupabaseEvent(
      "workflow",
      `Quality passed → auto-created shipping task #SH-${100 + (tickCount % 50)} (simulated)`,
      { source: "simulator" }
    );
  }
...
export function getSimulatorMachines(): MachineSpec[] {
  return MACHINES;
}
